...
# Tutorial: Displaying Git Branch in Your Shell Prompt
...
- `COLOR_GIT=$'%F{51}'`: Sets the color for the Git branch to **cyan** (color code 51 in Zsh’s 256-color palette).
...
- `export PROMPT='%n@%m %1~ ${COLOR_GIT}$(parse_git_branch)${COLOR_DEF} %# '`: Defines the prompt format:
  ...
  - `${COLOR_GIT}$(parse_git_branch)${COLOR_DEF}`: Displays the Git branch in **cyan**, resetting the color afterward.
...
- `COLOR_GIT="\[\033[38;5;51m\]"`: Sets the Git branch color to **cyan** using ANSI 256-color code 51.
...
- `$COLOR_GIT = "`e[38;5;51m"`: Defines the **cyan** color for the Git branch using ANSI 256-color code 51.
...
- `"${user}@${host} ${path} ${COLOR_GIT}${gitBranch}${COLOR_DEF} $ "`: Constructs the prompt:
  ...
  - Git branch in **cyan** (e.g., `[main]`), if available.
  - A `$` symbol at the end.
