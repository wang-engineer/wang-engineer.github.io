...
A **StorageClass** is a Kubernetes resource that defines the type of storage and its behavior. Think of it as a template that tells Kubernetes, “When someone asks for storage, give them *this* kind with *these* rules.”
...
- **reclaimPolicy**: Defines what happens to the underlying storage when the PV is deleted. Options are:
  - **Delete**: The PV and its backing storage (e.g., EBS volume) are deleted.
  - **Retain**: The PV is deleted, but the backing storage remains, allowing manual recovery or reuse.
